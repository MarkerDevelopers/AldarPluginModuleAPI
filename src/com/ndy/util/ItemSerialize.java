...
public class ItemSerialize {
    ...
    public static List<String> parseLore(String data){
        try {
            String loreValue[] = data.substring(data.indexOf('{') + 1, data.indexOf('}')).split(";");
            return Arrays.asList(loreValue);
        }catch (StringIndexOutOfBoundsException ex) {
            System.out.println("StringIndexOutofBoundsException : ItemSerialize line:93");
            return new ArrayList<>();
        }
    }

    public static boolean hasArray(String data){
